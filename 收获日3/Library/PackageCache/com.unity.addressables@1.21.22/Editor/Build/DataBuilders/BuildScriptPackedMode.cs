...
namespace UnityEditor.AddressableAssets.Build.DataBuilders
{
    ...
    public class BuildScriptPackedMode : BuildScriptBase
    {
        ...
        internal ReturnCode CreateCatalogBundle(string filepath, byte[] data, AddressablesDataBuilderInput builderInput)
        {
            ...
            if (builderInput.Target == BuildTarget.WebGL || builderInput.Target == BuildTarget.WeixinMiniGame)
            ...
            AddressableAssetsBuildContext aaContext)
        {
           ...
           if (loadPath.StartsWith("http://", StringComparison.Ordinal) && PlayerSettings.insecureHttpOption == InsecureHttpOption.NotAllowed)
            ...
            if (schema.Compression == BundledAssetGroupSchema.BundleCompressionMode.LZMA && aaContext.runtimeData.BuildTarget == BuildTarget.WeixinMiniGame.ToString())
                Addressables.LogWarning($"Addressable group {assetGroup.Name} uses LZMA compression, which cannot be decompressed on WeixinMiniGame. Use LZ4 compression instead.");
